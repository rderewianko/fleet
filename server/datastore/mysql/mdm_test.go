...
func testBulkSetPendingMDMHostProfiles(t *testing.T, ds *Datastore) {
	...
	// "break" the two G6 label-based profiles by nulling their label references
	// (FK is now RESTRICT so we can no longer delete a referenced label)
	simulateBrokenLabel(t, ds, ctx, labels[0].Name)
	simulateBrokenLabel(t, ds, ctx, labels[3].Name)
	...
	// "break" the team 2 label-based profile by nulling label references
	// (FK is now RESTRICT so we can no longer delete a referenced label)
	simulateBrokenLabel(t, ds, ctx, labels[1].Name)
	simulateBrokenLabel(t, ds, ctx, labels[4].Name)
